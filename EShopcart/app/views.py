...
class ProductView(View):
    def get(self,request):
     total_item=0
     subscriptions=Product.objects.filter(category='TW')
     bottomwears=Product.objects.filter(category='BW')
     paintings=Product.objects.filter(category='M')
     handcrafts=Product.objects.filter(category='L')
     if request.user.is_authenticated:
        total_item=len(Cart.objects.filter(user=request.user))
     return render(request,'app/home.html',{'subscriptions':subscriptions,'bottomwears':bottomwears ,'paintings':paintings ,'handcrafts':handcrafts ,'total_item':total_item})
...
def painting(request,data=None):
    ...
    if data == None:
        paintings=Product.objects.filter(category='M')
        active='btn-primary'
    elif data=='Happykidz' or data=='Others':
      paintings=Product.objects.filter(category='M').filter(brand=data)
      active='btn-primary'
    elif data=='below':
      paintings=Product.objects.filter(category='M').filter(discounted_price__lt=20)
      active='btn-primary'
    elif data=='above':
          paintings=Product.objects.filter(category='M').filter(discounted_price__gt=20)
          active='btn-primary'
    if request.user.is_authenticated:
        total_item=len(Cart.objects.filter(user=request.user))

    return render(request, 'app/painting.html',{'paintings':paintings, 'total_item':total_item ,'active':active ,'data': data})

def handcraft(request,data=None):
    total_item=0
    if data == None:
        handcrafts=Product.objects.filter(category='L')
        active='btn-primary'
    elif data=='ASUS' or data=='LENOVO':
      handcrafts=Product.objects.filter(category='L').filter(brand=data)
      active='btn-primary'
    elif data=='below':
      handcrafts=Product.objects.filter(category='L').filter(discounted_price__lt=60000)
      active='btn-primary'
    elif data=='above':
          handcrafts=Product.objects.filter(category='L').filter(discounted_price__gt=70000)
          active='btn-primary'
    if request.user.is_authenticated:
        total_item=len(Cart.objects.filter(user=request.user))

    return render(request, 'app/handcraft.html',{'handcrafts':handcrafts, 'total_item':total_item,'active':active,"data":data})

def subscription(request,data=None):
    total_item=0
    if data == None:
      subscriptions=Product.objects.filter(category='TW')
      active='btn-primary'
    elif data=='12-Months-Subscription' or data=='3-to-6-months-subscription':
      subscriptions=Product.objects.filter(category='TW').filter(brand=data)
      active='btn-primary'
    elif data=='below':
      subscriptions=Product.objects.filter(category='TW').filter(discounted_price__lt=400)
      active='btn-primary'
    elif data=='above':
          subscriptions=Product.objects.filter(category='TW').filter(discounted_price__gt=500)
          active='btn-primary'
    if request.user.is_authenticated:
        total_item=len(Cart.objects.filter(user=request.user))

    return render(request, 'app/subscription.html',{'subscriptions':subscriptions, 'total_item':total_item,'active':active,"data":data})
  ...
  #  post using cleaned_data
   def post(self,request):
      ...
      if form.is_valid():
         ...
         reg=Customer(user=usr,name=name,address=address,city=city,state=state,postcode=postcode)
         reg.save()
